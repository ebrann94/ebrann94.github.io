window.addEventListener("load", () => {
    ...
    op.knobWidth = op.knobWidth || op.knobDiameter || 40;
    op.knobHeight = op.knobHeight || op.knobDiameter || 40;
    ...
    let makeKnobFrames = (fr, fg, bg) => {
        let r = `
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="40" height="${
                fr * 64
            }" viewBox="0 0 64 ${fr * 64}" preserveAspectRatio="none">
            <defs>
                <g id="K">
                    <circle cx="32" cy="32" r="30" fill="${bg}" />
                    <circle cx="32" cy="32" r="27" opacity="0.75" fill="transparent" stroke="#000" stroke-width="2"/>
                    <line x1="32" y1="28" x2="32" y2="10" stroke-linecap="round" stroke-width="3" stroke="${fg}"/>
                </g>
            </defs>
        ...
        for (let i = 1; i < fr; ++i) {
            r += `
                <use
                    xlink:href="#K"
                    transform="translate(0,${64 * i}) rotate(${
                -135 + (270 * i) / fr
            },32,32)"
                />
            `;
    ...
    let initKnobs = (el) => {
        ...
        el.setValue = (v) => {
            ...
            if (v < ik.valrange.min) {
                v = ik.valrange.min;
            }

            if (v > ik.valrange.max) {
                v = ik.valrange.max;
            }
        ...
        el.redraw = (f) => {
            if (f || ik.valueold != el.value) {
                let v =
                    (el.value - ik.valrange.min) /
                    (ik.valrange.max - ik.valrange.min);
                if (ik.sprites >= 1)
                    el.style.backgroundPosition =
                        "0px " +
                        -((v * ik.sprites) | 0) * ik.frameheight +
                        "px";
                else {
                    switch (ik.itype) {
                        case "k":
                            el.style.transform =
                                "rotate(" + (270 * v - 135) + "deg)";
                            break;
                        case "h":
                            el.style.backgroundPosition =
                                (w - h) * v + "px 0px";
